...
def tickerSymboltoUpper(x):
    return x.upper()

def tickerSymbolcheckLength(x):
    if 1 <= len(x) <= 7:
        return True
    else:
        return False

def tickerSymbolAlpha(x):
    if x.isalpha():
        return True
    else:
        return False
...
def isChartCorrect(input):
    return input not in [1,2]
...
def timeSeriesInput():
    while True:
        try:
            ...
            if isTimeCorrect(chartTimeSeries):
